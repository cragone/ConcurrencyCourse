...
func main() {
	...
	fmt.Println("The table is empty.")
	for index, name := range finishedEating {
		fmt.Printf("%s was eater number %d\n", name, index+1)
	}
...
func dine() []string {
	...
	orderFinished := sync.Mutex{}
	// Start the meal by iterating through our slice of Philosophers.
	for i := 0; i < len(philosophers); i++ {
		// fire off a goroutine for the current philosopher
		go func() {
			phil := diningProblem(philosophers[i], wg, forks, seated)
			orderFinished.Lock()
			finishedEating = append(finishedEating, phil)
			orderFinished.Unlock()
		}()
	}
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup) string {
	defer wg.Done()
